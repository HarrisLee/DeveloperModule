...
- (id)init {
    ...
    if (self) {
        ...
        CGRect viewFrame = CGRectMake(0, 20, mainViewFrame.size.width, mainViewFrame.size.height-20);
        contentView = [[UIView alloc] initWithFrame:viewFrame];
        contentView.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageWithName:@"BG_INDEX"]];

        imageScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, mainViewFrame.size.width, mainViewFrame.size.height-20)];
        ...
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, viewWithNothing/2 - 360, 320, 180)];
        [imageView setImage:[UIImage imageWithName:@"LOADING3"]];
        ...
        footBack.image = [UIImage imageWithName:@"floatbar.png"];
        ...
        shareImg.image = [UIImage imageWithName:@"share_icon.png"];
...
-(CGRect)CGRectForOrientation{
    ...
    if (orientation == UIInterfaceOrientationLandscapeLeft) {
        return CGRectMake(0, 0, mainScreenBounds.size.height, mainScreenBounds.size.width);
...
- (void)downloadImage {
    ...
    if (!imageSaved) {
        ...
        [[HttpRequestCenter shareInstance]showMaskWithTimer:60 onView:contentView];
    }
    else {
        msg = @"图片已被存储，请勿重复操作";
    }
    if (msg) {
        CAlertView *alert = [[CAlertView alloc] initWithTitle:@"图片存储"
                                                        message:msg
                                                       delegate:nil
  ...
  contextInfo:(void *)contextInfo
{
    ...
    if (error != NULL)
    {
        msg = @"请在“设置-隐私-照片”中打开“天禄奢品”的照片访问权限";
    }
    else  // No errors
    {
        imageSaved = YES;
